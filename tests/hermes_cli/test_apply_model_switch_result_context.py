...
straight from models.dev. That ignored provider-specific context metadata. The
sibling ``_handle_model_switch()`` (typed ``/model <name>``) was already fixed
to use ``resolve_display_context_length()``; the picker path was missed, causing
inconsistent context display for the same model across sibling UI paths.
...
def test_picker_path_uses_provider_aware_context_on_codex(monkeypatch):
    ...
    over stale or under-reported model catalog values.
    """
    ...
    assert "1,050,000" in ctx_line, (
        f"picker-path display must show Codex gpt-5.5's 1.05M context, got: {ctx_line!r}"
    )
