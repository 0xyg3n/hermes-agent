...
class TestCodexOAuthContextLength:
    """ChatGPT Codex OAuth has provider-specific context limits. Most slugs
    stay at the Codex catalog value, while gpt-5.5 explicitly supports the
    full 1.05M OAuth window.
    """
    ...
    def test_live_probe_overrides_fallback_except_explicit_overrides(self):
        """When a token is provided, the live /models probe is preferred
        unless an explicit slug override exists."""
        from agent.model_metadata import get_model_context_length
        ...
        assert ctx_55 == 1_050_000
        assert ctx_54 == 400_000

    def test_probe_failure_falls_back_to_hardcoded(self):
        """If the probe fails, gpt-5.5 still gets the explicit OAuth override."""
        from agent.model_metadata import get_model_context_length
    ...
    def test_stale_codex_cache_over_400k_is_invalidated(self, tmp_path, monkeypatch):
        """Pre-PR #14935 builds cached non-overridden gpt-5.x slugs at 1.05M
        from models.dev. Those stale entries must still be dropped and
        re-resolved through the live Codex metadata path.
        """
        ...
        cache_file.write_text(_yaml.dump({"context_lengths": {
            stale_key: 1_050_000,   # stale pre-fix value for a non-overridden slug
        ...
        mock_save.assert_called_with("gpt-5.4", base_url, 272_000)
    ...
    def test_gpt55_override_replaces_underreported_codex_cache(self, tmp_path, monkeypatch):
        """gpt-5.5 supports 1.05M on Codex OAuth, so stale 272k cache entries
        must be replaced by the explicit override without trusting /models.
        """
        from agent import model_metadata as mm
    ...
    def test_fresh_codex_cache_under_400k_is_respected(self, tmp_path, monkeypatch):
        """Non-overridden Codex entries at the correct 272k must NOT be
        invalidated, only stale pre-fix values (>= 400k) get dropped."""
        from agent import model_metadata as mm
